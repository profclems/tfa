...
2FA_VERSION ?= $(shell git describe --tags 2>/dev/null || git rev-parse --short HEAD)
DATE_FMT = +%Y-%m-%d
...
GOURL ?= github.com/profclems/tfa
GO_LDFLAGS := -X $(GOURL)/internal/build.Date=$(BUILD_DATE) $(GO_LDFLAGS)
GO_LDFLAGS := $(GO_LDFLAGS) -X $(GOURL)/internal/build.Version=$(2FA_VERSION)
BUILDLOC ?= ./bin/tfa
...
.DEFAULT_GOAL := build
build:
	go build -trimpath -ldflags "$(GO_LDFLAGS)" -o $(BUILDLOC) $(GOURL)/cmd/tfa

clean: ## Clear the working area and the project
	rm -rf ./bin ./.tfa-cli ./test/testdata-* ./coverage.txt coverage-*
.PHONY: clean

.PHONY: install
install: ## Install tfa in $GOPATH/bin
	GO111MODULE=on go install -trimpath -ldflags "$(GO_LDFLAGS)" $(GOURL)/cmd/tfa

.PHONY: run
run:
	go run -trimpath -ldflags "$(GO_LDFLAGS)" ./cmd/tfa $(cmd)
...
gen-docs: ## Generate docs
	...
	#cp ./docs/tfa.rst ./docs/index.rst
